...
const layoutProps = ['id', 'layout', 'label', 'tooltip', 'required', 'errors', 'hints', 'className', 'style'];
...
const FormField = ({type, name, children, fast, ...rest}) => (name ? (
    <Field {...rest} type={type} name={name}>
        {({field, form, meta}) => {
            const {name} = field;
            const {handlChange, validateOnMount} = form;
            ...
            return (
                <Layout {...rest} id={name} errors={errors}>
