import { createSlice, createAsyncThunk, PayloadAction, AsyncThunk, ThunkArg } from '@reduxjs/toolkit'
...
type reducerFunction =(state: NoahState, action: PayloadAction<NoahState>) => void
...
export const getNoahList = createAsyncThunk(
  ...
  async (payload: Payload) => {
    ...
    if (code === REQUEST_CODE.SUCCESS) {
      ...
      return {
        list: list.map((item: any) => ({ ...item, key: item.id })),
        total: total,
        currentPage,
        type
      }
...
const init: reducerFunction = (state, action) => {
  const { list, currentPage, total } = action.payload
  state.list = list
  state.currentPage = currentPage
  state.total = total
}
const loadMore: reducerFunction = (state, action) => {
...
const noahSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [getNoahList.pending]: (state: NoahState) => {
      state.loading = true
    },
    [getNoahList.fulfilled]: (state: NoahState, action: PayloadAction<NoahState>) => {
