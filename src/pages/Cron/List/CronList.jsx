...
const CronList = props => {
    ...
    const columns = [{
        title: '任务名称', dataIndex: 'cronExecute', render(val) {
            return propOr(DEFAULT_STRING_VALUE, 'taskName', val);
        },
    }, {
        title: '作业名称', dataIndex: 'workPlanName',
    }, {
        title: '策略类型', dataIndex: 'cronExecute', render: val => {
            return STRATEGIES_TYPES.get(val?.exePolicy)?.label;
        },
    }, {
        title: '更新人', dataIndex: 'cronExecute', render: val => {
            return propOr(DEFAULT_STRING_VALUE, 'userName', val);
        },
    }, {
        title: '最新执行结果', dataIndex: 'latestWorkTrigger', align: 'center', render(status) {
            if (!status) {
        ...
        title: '最近执行时间', dataIndex: 'latestWorkTrigger', align: 'center', render(val) {
            return formatTimeStamp(prop('beginTime', val));
        },
    }, {
        title: '操作', width: 220, align: 'center', render: (_, record) => {
            const {
                cronExecute: {exeStatus} = null,
            } = record;
            /*
